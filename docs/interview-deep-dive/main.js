...
(function () {
  ...
  function initDnD(containerEl) {
    ...
    zones.forEach(zone => {
      ...
      target.addEventListener('drop', e => {
        ...
        placeChip(target, $(`.dnd-chip[data-answer="${e.dataTransfer.getData('text/plain')}"]`, containerEl));
      });
    });
    ...
    chips.forEach(chip => {
      ...
      chip.addEventListener('touchend', e => {
        ...
        const zt    = el && el.closest('.dnd-zone-target');
        if (zt) {
          chip._answer = chip._answer || chip.dataset.answer;
          placeChip(zt, chip);
        }
        zones.forEach(z => { const t = $('.dnd-zone-target', z); if (t) t.classList.remove('drag-over'); });
      });
  ...
  // A zone target can hold several chips; dataset.placed keeps space-separated answers.
  // Zones may declare comma-separated correct answers in data-correct.
  function placeChip(target, chip) {
    if (!chip || !target) return;
    const answer = chip.dataset.answer;
    const placed = (target.dataset.placed || '').split(' ').filter(Boolean);
    if (placed.includes(answer)) return;
    if (!placed.length) target.innerHTML = '';
    const tag = document.createElement('span');
    tag.className = 'dnd-placed-chip';
    tag.textContent = chip.textContent;
    target.appendChild(tag);
    target.dataset.placed = placed.concat(answer).join(' ');
  ...
  window.checkDnD = function (containerId) {
    ...
    $$('.dnd-zone', container).forEach(zone => {
      ...
      const correctIds = (zone.dataset.correct || '').split(',').map(s => s.trim());
      const placedArr  = target.dataset.placed.split(' ').filter(Boolean);
      const allCorrect = placedArr.length > 0 && placedArr.every(a => correctIds.includes(a));
