...
> **2026-09-11 状态补充**：本表登记的是更早一轮的 F 系列自查项；全场景测试暴露的
> **N 系列系统侧缺陷（N18/N19/N22/N23/N25）已于 2026-09-11 全部修复并经真机复验**
> （见 [known-issues.zh-CN.md](known-issues.zh-CN.md) §23），**当前系统侧未修缺陷为 0**；
> 唯一的高优先级遗留是**上游依赖 N27**（vLLM 流式工具调用参数丢末字符），非本项目代码缺陷。
